...
def plotCaustic(s,xMin=-4.0,xMax=4.0):
	...
	plot.plot(yVals,xVals)
	plot.xlabel("Distance from Caustic (Units of Caustic 'Strength')")
	plot.ylabel("Magnification of Source")
	plot.show()
...
def plotCaustics(rects,xMin = -4.0, xMax = 4.0):
	...
	for gp in gps.values():
		...
		plot.plot(xVals,gp.yVals,label = gp.rects[0].group)
	plot.legend()
	plot.xlabel("Distance from Caustic (Units of Caustic 'Strength')")
	plot.ylabel("Magnification of Source")
	plot.show()
...
def plotAnnular(s,s2,xMin = -4.0, xMax = 4.0):
	...
	plot.plot(xVals,yValsLg,label = "Larger Square")
	plot.plot(xVals,yValsSm,label = "Smaller Square")
	plot.legend()
	plot.xlabel("Distance from Caustic (Units of Caustic 'Strength')")
	plot.ylabel("Magnification of Source")
	plot.show()
