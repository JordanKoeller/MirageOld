...
class PySparkCalculationDelegate(CalculationDelegate):
    '''
    classdocs
    '''
    ...
    def ray_trace(self):
        ...
        arr = np.ndarray((_width*2,_height*2,2))
        for i in range(-_width,_width):
            for j in range(-_height,_height):
                arr[i,j] = [i,j]
        print("Finished building the array")
        from .RayTracerDelegate import ray_trace
        rdd = self.sc.parallelize(arr)
        rdd2 = rdd.glom().map(np.array)
        rdd3 = rdd2.map(lambda partition: ray_trace(partition,parameters))

        print("Now going to collect")
        ret = rdd3.collect()
        print("Done")
