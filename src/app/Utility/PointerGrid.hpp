...
//    typedef  unsigned int Index;
    ...
    struct Node
    {
        ...
        void queryNode_sz( double& ptx,  double& pty, double &r2, PointerGrid* prnt, unsigned int & ret) 
        {
            for (Index i = 0; i < node_data.size(); ++i)
            {
                if (within(ptx,pty,r2,node_data[i],prnt))
        ...
        void takeAll_sz(PointerGrid* prnt, unsigned int & ret)
        {
            ret += node_data.size();
        }
...
//              cout << x << " , " << y << " ==> " << i << "\n";
    ...
    unsigned int find_within_count (double &x,  double &y,  double &r) override
    {
        ...
        if (cx >= 0 && cx < data.size() && cy >= 0 &&  cy < data[0].size())  {
            data[cx][cy].queryNode_sz(x,y,r2,this,ret);
        }
        for (Index i=1; i <= rx; i++) {
            if (cx+i >= 0 && cx+i < data.size() && cy >= 0 && cy < data[0].size())  {
                data[cx+i][cy].queryNode_sz(x,y,r2,this,ret);
            }
            if (cx-i >= 0 && cx-i < data.size() && cy >= 0 && cy < data[0].size())  {
                data[cx-i][cy].queryNode_sz(x,y,r2,this,ret);
            }
        ...
        for (Index i=1; i <= ry; i++) {
            if (cx >= 0 && cx < data.size() && cy+i >= 0 && cy+i < data[0].size())  {
                data[cx][cy+i].queryNode_sz(x,y,r2,this,ret);
            }
            if (cx >= 0 && cx < data.size() && cy-i >= 0 && cy-i < data[0].size())  {
                data[cx][cy-i].queryNode_sz(x,y,r2,this,ret);
            }
        ...
        for (Index i = 1; i <= rx; ++i) // Possible indexing issue here?
        {
            ...
            for (Index j = 1; j <= ryLow;++j) //Improvement by using symmetry possible
            {
                if ((i+1)*NODE_WIDTH*(i+1)*NODE_WIDTH + (j+1)*NODE_HEIGHT*(j+1)*NODE_HEIGHT < r2) {
                    if (i+cx >= 0 && i+cx < data.size() && cy+j >= 0 && cy+j < data[0].size())  {
                        data[i+cx][j+cy].takeAll_sz(this,ret);
                    }
                    if (cx-i >= 0 && cx-i < data.size() && cy+j >= 0 && cy+j < data[0].size()) {
                        data[cx-i][cy+j].takeAll_sz(this,ret);
                    }
                    if (cx+i >= 0 && cx+i < data.size() && cy-j >= 0 && cy-j < data[0].size()) {
                        data[i+cx][cy-j].takeAll_sz(this,ret);
                    }
                    if (cx-i >= 0 && cx-i < data.size() && cy-j >= 0 && cy-j < data[0].size()) {
                        data[cx-i][cy-j].takeAll_sz(this,ret);
                    }
                ...
                {
                    if (i+cx >= 0 && i+cx < data.size() && cy+j >= 0 && cy+j < data[0].size()) {
                        data[i+cx][j+cy].queryNode_sz(x,y,r2,this,ret);
                    }
                    if (cx-i >= 0 && cx-i < data.size() && cy+j >= 0 && cy+j < data[0].size()) {
                        data[cx-i][cy+j].queryNode_sz(x,y,r2,this,ret);
                    }
                    if (cx+i >= 0 && cx+i < data.size() && cy-j >= 0 && cy-j < data[0].size()) {
                        data[i+cx][cy-j].queryNode_sz(x,y,r2,this,ret);
                    }
                    if (cx-i >= 0 && cx-i < data.size() && cy-j >= 0 && cy-j < data[0].size()) {
                        data[cx-i][cy-j].queryNode_sz(x,y,r2,this,ret);
                    }
