...
__kernel void ray_trace(
		...
		__global double *result_buf_y)
	{
		...
		// Lensing from shear
	//	double phi = 2*(shear_angle+M_PI_2) - atan2(deltaR_y,deltaR_x); // Old shear angle definition, of east being left of north
		double phi = 2*(M_PI_2 - shear_angle) - atan2(deltaR_y,deltaR_x); //New shear angle definition, measured east of north.
