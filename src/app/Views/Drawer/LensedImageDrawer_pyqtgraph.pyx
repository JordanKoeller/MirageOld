...
#         return self.drawImage(canvas,None)
    ...
    cdef void __drawTrackers(self,np.ndarray[np.uint8_t,ndim=3] canvas, object parameters): #*************NOT OPTIMIZED **************
        ...
        for i in range(-1,1):
            for j in range(-1,1):
                canvas[i+xInt.x+ int(parameters.canvasDim/2)][int(parameters.canvasDim/2) - (j+xInt.y)] = lookup[3]
