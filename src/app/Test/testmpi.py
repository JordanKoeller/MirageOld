...
if comm.Get_rank() == 0:
	...
	data = comm.irecv(source=ProcessRole.MAIN_PROCESS.value,tag=SignalCode.DEFAULT.value)
	print("Received Data")
	sendSignal(ProcessRole.RECORD_PROCESS,SignalCode.START)
elif comm.Get_rank() == 2:
	data = comm.irecv(ProcessRole.CALCULATION_PROCESS.value,tag=SignalCode.START.value)
	print("Done on com 2")
